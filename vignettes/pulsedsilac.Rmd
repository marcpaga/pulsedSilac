...
- Optimize modelTurnover code
- Compare plots
- Add rownames and colnames support
- Add support for one condition 
- Improve documentation on base generics
- Improve repetitive code
- Add more comments
- Add coercers
- Improve getMetaoptions system
- Add more tests
...
This package contains a set of functions to analyze pulsed SILAC quantitative data. Functions are provided to organize the data , calculate isotope ratios and fractions, model protein turnover, compare turnover models, estimate cell growth and estimate isotope recycling. Several visualization tools are also included to explore the data for QC and filtering; and the results to compare conditions, replicates and models.
...
overlapFeatures(ProtExp(wormsPE2), 
                assayName = 'ratio', 
                maxMissing = 2)
...
Before estimating protein turnover it might be wise to visualize the data that is going to be used. For that purpose there are two functions `plotAssay()` and `compareAssay()`. The first one will plot the distributions of the data for selected assay for each timepoint and condition. The latter will plot each protein/peptide value of a condition against the other for each timepoint.

## `plotAssay()`

```{r, warning = FALSE}

plotAssay(wormsPE, assayName = 'fraction')

```

From this plot it can already be seen that the OW450 line has a much faster heavy label incorporation that the OW40 line. Moreover, it looks like the 13h timepoint has a technical deviation.

## `compareAssay()`

For `compareAssay()` it is necessary to separate the object into the two conditions that should be compared. The grey dash line indicates the identity line. To make it simpler the only the protein data will be used.

```{r}

protPE <- ProtExp(wormsPE)

compareAssay(x = protPE[,1:7], 
             y = protPE[, 8:14], 
             assayName = 'ratio')
...
plotDistribution(modelList, value = 'aicvalue')
...
plotDistribution(modelList = modelProbabilities, 
...
For the current dataset, cell division was not an issue since adult worms do no longer divide. To illustrate the effects of cell growth we will use data of mouse embryonic fibroblasts (MEFs) cultured in two medium conditions: with and without serum. MEFs that are cultured with serum will grow at a "normal" rate while MEFs cultured without serum will grow at a significant slower rate.
...
plotAssay(mefPE, assayName = 'fraction')

```

## Most stable proteins

To extract the most stable proteins the `mostStable()` function can be used. This function will rank the incorporation on new label accross time and conditions, and then select the top `n` proteins/peptides. 
...
plotDistribution(stableModels, value = 'parameter', plotType = 'boxplot')
...
Next the turnover rates can be calculated using the doubling time correction model. Because condition has a different "model", these need to be run separately.
...
plotDistribution(modelList = modelsMef,
                 value = 'parameter',
                 plotType = 'density') 
...
plotDistribution(modelList = modelsNoCorrect,
                 value = 'parameter',
                 plotType = 'density') 
...
Once a protein is degraded, its amino acids can be recycled to synthesize new proteins. This process will have an effect on protein turnover measurements. For example, light label amino acids will be used to synthesize new proteins, but will be interpreted as being old proteins. This results in an overall understimation of protein turnover.

To estimate how much "old" label is being used in new proteins misscleaved peptides can be used. If a misscleaved peptide contains both light and heavy label, it means that light lysine was used for a new protein. Using the following formula the light label recycling can be estimated.

$$ \frac{1}{2\frac{Intensity_{lys8lys8}}{Intensity_{lys8lys0}} + 1} = lys_0 (Fraction)$$

Where Intensity<sub>lys8lys8</sub> is the intensity of misscleaved peptides containing two heavy isotope lysines and Intensity<sub>lys8lys0</sub> is the intensity of misscleaved peptides containing one heavy isotope lysine and one light isotope lysine.
...
There are two assays in the `missPE` object. These contain the intensities for peptides that contain two heavy isotope lysines (2 Copy of Lys8) and intensities for peptides that contain one heavy and one light isotope lysines (Copy of Lys 8). 

## Calculating light label recycling

To calculate the amount of light label used in protein synthesis the `calculateOldIsotopePool()` function can be used. This function applies the formula mentioned above. This function adds a new assay with the name of `oldIsotopePool`.
...
With this, the supplemental figure 6B of @Visscher can be reproduced.
