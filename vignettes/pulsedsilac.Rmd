...
- Add more tests
- Add SummarizedExperiment to our experiments coercer
- Add function to calculate half life (rootSolve)
...
For `compareAssay()` it is necessary to separate the object into the two conditions that should be compared. The grey dash line indicates the identity line. To make it simpler, only the protein data will be used.
...
To plot the overall distributions of the model metrics per condition the `plotDistribution` function can be used. This can show global changes between conditions and/or can be used for quality control. Some metrics that might be interesting to check are the R<sup>2</sup> error of the models, the estimated parameters, the residuals and the weights (only for robust modelling).
