...
To visualize data stored in the `assays` slot there are two functions `boxplotAssay()` and `scatterCompareAssays()`. The first one will plot the distributions of the data for a selected assay for each timepoint and condition. The latter will plot each protein/peptide value of a condition against the other for each timepoint.

## `boxplotAssay()`

```{r, warning = FALSE}

boxplotAssay(wormsPE, assayName = 'fraction')
...
For `scatterCompareAssays()` it is necessary to separate the object into the two conditions that should be compared. The grey dash line indicates the identity line. To make it simpler, only the protein data will be used.

```{r}

protPE <- ProtExp(wormsPE)

scatterCompareAssays(x = protPE[,1:7], 
                     y = protPE[,8:14], 
                     assayName = 'ratio')
...
To plot protein/peptide values 
...
plotDistributionModel(modelList, value = 'aicvalue')
...
boxplotAssay(mefPE, assayName = 'fraction')
...
plotDistributionModel(stableModels, value = 'parameter', plotType = 'boxplot')
